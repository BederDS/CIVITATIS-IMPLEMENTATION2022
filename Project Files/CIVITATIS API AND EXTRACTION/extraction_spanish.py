...
usr = 'bederapp'
pss ='w5OWhwNPL1tU'
lang = 'es' # cambiar acá idioma
session = civi.Civitatis(username=usr,password=pss)
today = date.today()
d1 = today.strftime("%d_%m_%Y")


civitatis_countries = session.get_countries('es')
civitatis_countries_df = pd.DataFrame(civitatis_countries)

civitatis_countries_df.to_csv(f'C:\\Users\\beder\Desktop\Data 2022\Projects\CIVITATIS-IMPLEMENTATION2022\Outputs\Civitatis Exctraction\civitatis_countries{d1}_es.csv',index=False)
...
all_destinations_df =  pd.DataFrame(session.get_destinations('es'))

all_destinations_df.to_csv(f'C:\\Users\\beder\Desktop\Data 2022\Projects\CIVITATIS-IMPLEMENTATION2022\Outputs\Civitatis Exctraction\\all_destinations{d1}_es.csv',index=False)

acti_frames= []
for i, acti_id  in enumerate(all_destinations_df['id']):

    print(acti_id,i,len(all_destinations_df['id']),flush=True)
    acti_frames.append(pd.DataFrame(session.get_destination_activities(acti_id,'es')))
...
longitude = []
latitude = []
for act in all_activities['address']:
    longitude.append(act['longitude'])
    latitude.append(act['latitude'])

all_activities['longitude'] =  longitude
all_activities['latitude'] =  latitude
all_activities.to_csv(f'C:\\Users\\beder\Desktop\Data 2022\Projects\CIVITATIS-IMPLEMENTATION2022\Outputs\Civitatis Exctraction\\all_activities{d1}_es.csv',index=False)
...
countries_col = countries['spanish_name'].unique()# change here
dic_replace = {}
...
initial_merge = pd.merge(countries,civitatis_countries_df,how='left', left_on='spanish_name',right_on='name') # change here
...
countries_merged.to_csv(f'C:\\Users\\beder\Desktop\Data 2022\Projects\CIVITATIS-IMPLEMENTATION2022\Outputs\Civitatis Exctraction\\civitatis_mapped_countries{d1}_es.csv',index=False)